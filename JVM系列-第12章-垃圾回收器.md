...
![image-20200713083030867](E:\JavaLearning\JVM-Note\JVM系列-第12章-垃圾回收器.img\e2af28473690473aa86e591a970251a2)
...
![image-20200713083443486](E:\JavaLearning\JVM-Note\JVM系列-第12章-垃圾回收器.img\f5376dafb6574023b68f4e3d85690e35)
...
![image-20220104222138796](img/image-20220104222138796.png)
...
![image-20220104222149999](img/image-20220104222149999.png)
...
- 低暂停时间（低延迟）较好，是从最终用户的角度来看，不管是GC还是其他原因导致一个应用被挂起始终是不好的。这取决于应用程序的类型，**有时候甚至短暂的200毫秒暂停都可能打断终端用户体验**。因此，具有较低的暂停时间是非常重要的，特别是对于一个**交互式应用程序**。
...
![image-20200713093551365](E:\JavaLearning\JVM-Note\JVM系列-第12章-垃圾回收器.img\b20873e989cd4337913a0330d42969e9)

**官方文档**

![image-20220105100740100](img/image-20220105100740100.png)

**7款经典回收器与垃圾分代之间的关系**

![image-20220105100900442](img/image-20220105100900442.png)
...
![image-20220105100932595](img/image-20220105100932595.png)
...
![image-20220105115523189](E:\JavaLearning\JVM-Note\JVM系列-第12章-垃圾回收器.img\image-20220105115523189.png)

#### 2.JDK9

<img src="E:\JavaLearning\JVM-Note\JVM系列-第12章-垃圾回收器.img\0010.png">

![image-20220105115844468](E:\JavaLearning\JVM-Note\JVM系列-第12章-垃圾回收器.img\image-20220105115844468.png)
...
这个收集器是一个单线程的收集器，“单线程”的意义：它**只会使用一个CPU（串行）或一条收集线程去完成垃圾收集工作（即使在多核的环境下）**。更重要的是在它进行垃圾收集时，**必须暂停其他所有的工作线程**，直到它收集结束（Stop The World）

![image-20200713100703799](E:\JavaLearning\JVM-Note\JVM系列-第12章-垃圾回收器.img\c9a3ac2f93ff431dae0a5396dec54667)
...
![image-20200713102030127](E:\JavaLearning\JVM-Note\JVM系列-第12章-垃圾回收器.img\7956256d195d42c882ea2efb9bd9078b)
...
2. 但是在**单个CPU的环境下，ParNew收集器不比Serial收集器更高效。**虽然Serial收集器是基于串行回收，但是由于CPU不需要频繁地做任务切换，因此可以有效避免多线程交互过程中产生的一些额外开销。
...
- Parallel Old收集器采用了**标记-压缩**算法，但同样也是基于**并行回收和"Stop-the-World"**机制。
  ...
  ![image-20200713110359441](E:\JavaLearning\JVM-Note\JVM系列-第12章-垃圾回收器.img\df20d6f88ba744faa9f2eb12c5c4bb55)
...
- **-XX:+UseAdaptiveSizePolicy**    设置Parallel Scavenge收集器具有**自适应调节策略**
   - 在这种模式下，年轻代的大小、Eden和Survivor的比例、晋升老年代的对象年龄等参数会被自动调整，已达到在堆大小、吞吐量和停顿时间之间的平衡点。（比如年轻代 eden : to : from 默认比例为8:1:1，但是实际上是6:1:1，就是因为自适应调节策略，不过把这个参数关了也不行，要显示的指定堆的：-XX:SurvivorRatio=8）
...
![image-20200713205154007](E:\JavaLearning\JVM-Note\JVM系列-第12章-垃圾回收器.img\68b528d2d17a4aaca3550358d35380e7)
