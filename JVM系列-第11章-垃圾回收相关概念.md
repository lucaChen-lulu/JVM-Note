...
public class LocalVarGC {
    ...
    //buffer不会被回收，局部变量表最大槽数有两个：this 和 buffer(不会显示在局部变量表中)。第二个slot被buffer所占据，所以引用还在。
    ...
    //value这个局部变量会复用buffer的slot，导致buffer的引用不存在，会被回收。
    ...
    //第一个方法的栈帧弹出，该方法的局部变量表也随之销毁，buffer的引用断开，在第二次gc中会被回收。
...
- 尽管内存泄漏并不会立刻引起程序崩溃，但是一旦发生内存泄漏，程序中的可用内存可能就会被逐步蚕食，直至耗尽所有内存，最终出现OutofMemory异常，导致程序崩溃。
...
左边的图：jvm使用可达性分析算法，最上面的数据不可达，就是需要被回收的对象。
...
![img](https://img2020.cnblogs.com/blog/985599/202105/985599-20210531205428162-875170986.png)
...
![img](https://img2020.cnblogs.com/blog/985599/202105/985599-20210531205448412-2138263048.png)

在垃圾回收期间，所以的CPU核心均指向GC线程，用户线程暂停。

#### **4.垃圾回收的并发**

并发是垃圾回收线程和用户线程同时执行，也是在多核CPU环境下，垃圾回收线程和用户线程并发执行，也就是同一个时刻CPU0上执行用户线程，CPU1上有可能执行垃圾回收线程；在垃圾回收期间，在多核CPU下，每个CPU执行的线程可能是不一样的，达到GC线程和用户线程并发执行的目的。（如：CMS,G1)

![img](https://img2020.cnblogs.com/blog/985599/202105/985599-20210531205631000-880118112.png)
...
- **抢先式中断：（目前没有虚拟机采用）**
  ...
  首先先中断所有线程。如果还有线程不在安全点，就恢复线程，让线程跑到安全点。

- **主动式中断：**

  设置一个中断标志，各个线程运行到Safe Point的时候**主动轮询**这个标志，如果中断标志为真，则将自己进行中断挂起。

### **2.安全区域（Safe Region）**

Safepoint 机制保证了程序执行时，在不太长的时间内就会遇到可进入GC的Safepoint。

但是，程序“不执行”的时候呢？例如线程处于Sleep状态或Blocked 状态，这时候线程无法响应JVM的中断请求，“走”到安全点去中断挂起，JVM也不太可能等待线程被唤醒。
...
- 【既**偏门（使用频率低）**又非常**高频**的面试题：强引用、软引用、弱引用、虚引用有什么区别？具体使用场景是什么？

  

- 在JDK1.2版之后，Java对引用的概念进行了扩充，将引用分为：（这4种引用强度依次逐渐减弱）
...
本例中的两个引用（str, str1），都是强引用，强引用具备以下特点：
...
- 软引用是用来描述一些还有用，但非必需的对象。**只被软引用关联着的对象，在系统将要发生内存溢出异常前，（将不可达对象全部回收后，发现内存还是不够）会把这些对象列进回收范围之中进行第二次回收**，如果这次回收还没有足够的内存，才会抛出内存溢出异常。
...
//            byte[] b = new byte[1024 * 1024 * 7];
            //下面这行代码是分析老年代中可用空间后，将老年代空间尽可能撑大，以达到不报OOM，也将软引用回收的效果
...
**2.在内存紧张，但是不报OOM的情况下，也有可能会被回收**

```
[id=1, name=songhk] 
---目前内存还不紧张---
[GC (System.gc()) [PSYoungGen: 2000K->496K(2560K)] 2000K->752K(9728K), 0.0006012 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[Full GC (System.gc()) [PSYoungGen: 496K->0K(2560K)] [ParOldGen: 256K->676K(7168K)] 752K->676K(9728K), [Metaspace: 3496K->3496K(1056768K)], 0.0038948 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
After GC:
[id=1, name=songhk] 
---下面开始内存紧张了---
[GC (Allocation Failure) [PSYoungGen: 36K->64K(2560K)] 713K->740K(9728K), 0.0003437 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[GC (Allocation Failure) [PSYoungGen: 64K->64K(2560K)] 740K->740K(9728K), 0.0002619 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[Full GC (Allocation Failure) [PSYoungGen: 64K->0K(2560K)] [ParOldGen: 676K->670K(7168K)] 740K->670K(9728K), [Metaspace: 3496K->3496K(1056768K)], 0.0042489 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[GC (Allocation Failure) [PSYoungGen: 0K->0K(2560K)] 670K->670K(9728K), 0.0002863 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
[Full GC (Allocation Failure) [PSYoungGen: 0K->0K(2560K)] [ParOldGen: 670K->651K(7168K)] 670K->651K(9728K), [Metaspace: 3496K->3496K(1056768K)], 0.0041004 secs] [Times: user=0.00 sys=0.00, real=0.00 secs] 
null     <-没有OOM,但是软引用被回收了           
Heap
 PSYoungGen      total 2560K, used 61K [0x00000000ffd00000, 0x0000000100000000, 0x0000000100000000)
  eden space 2048K, 3% used [0x00000000ffd00000,0x00000000ffd0f7a8,0x00000000fff00000)
  from space 512K, 0% used [0x00000000fff80000,0x00000000fff80000,0x0000000100000000)
  to   space 512K, 0% used [0x00000000fff00000,0x00000000fff00000,0x00000000fff80000)
 ParOldGen       total 7168K, used 7159K [0x00000000ff600000, 0x00000000ffd00000, 0x00000000ffd00000)   <-老年代资源很紧张
  object space 7168K, 99% used [0x00000000ff600000,0x00000000ffcfdf08,0x00000000ffd00000)
 Metaspace       used 3503K, capacity 4502K, committed 4864K, reserved 1056768K
...
- 第一次尝试获取虚引用的值，发现无法获取的，这是因为无法通过虚引用来获取被引用的对象。
- 进行第一次GC，因为会调用finalize方法，将对象复活了，所以对象没有被回收
- 但是调用第二次GC操作的时候，因为finalize方法只能执行一次，对象无法复活，被GC回收了。同时GC发现它还有虚引用，就将这个虚引用加入引用队列，以通知应用程序对象的回收情况。（追踪垃圾回收过程：PhantomReferenceTest实例被GC了）
