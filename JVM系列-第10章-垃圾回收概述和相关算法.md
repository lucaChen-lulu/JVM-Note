...
![image-20220101155803734](img/image-20220101155803734.png)
...
使用JVisualVM捕捉 heap dump，并且使用MAT进行查看

代码：
*   numList 和 birth 在第一次捕捉内存快照的时候，为 GC Roots
*   之后 numList 和 birth 置为 null ，在第二次捕捉内存快照的时候，就不再是 GC Roots
...
新生代里面就用到了复制算法，Eden区和form区存活对象整体复制到to区
...
- 如果系统中的存活对象很多，复制算法就不会很理想。因为复制算法要求复制的存活对象数量要少。
...
- 标记-清除算法的确可以应用在老年代中，但是该算法不仅执行效率低下，而且在执行完内存回收后还会产生**内存碎片**，所以JVM的设计者需要在此基础之上进行改进。标记-压缩（Mark-Compact）算法由此诞生。
...
- 可以看到，标记的存活对象将会被整理，按照内存地址依次排列，而未被标记的内存会被清理掉。如此一来，当我们需要给新对象分配内存时，JVM只需要持有一个内存的起始地址即可，这比维护一个空闲列表显然少了许多开销。**（不用维护空闲列表，使用指针碰撞**）**
